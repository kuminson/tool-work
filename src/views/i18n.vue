<template>
  <div class="i18n">
    ...
    <div class="row">
      <a-upload :file-list="fileList" :remove="(file) => {handleRemove(file, 'fileList')}" :before-upload="(file) => {beforeUpload(file, 'single', 'fileList'); return false;}" accept=".xlsx">
    ...
    <div class="note">| key | en | ja | es | ko |</div>
    <div class="row">
      <a-upload :file-list="vueXlsx" :remove="(file) => {handleRemove(file, 'vueXlsx')}" :before-upload="(file) => {beforeUpload(file, 'single', 'vueXlsx'); return false;}">
        <a-button><a-icon type="upload" />选择xlsx文件</a-button>
      </a-upload>
    </div>
    <div class="row">
    ...
    <div class="row">
      <a-upload :file-list="vueDirectory" :remove="(file) => {handleRemove(file, 'vueDirectory')}" :before-upload="(file) => {beforeUpload(file, 'single', 'vueDirectory'); return false;}">
        <a-button><a-icon type="upload" />选择json文件</a-button>
      </a-upload>
    </div>
    <div class="row">
    ...
    methods: {
      ...
      downloadXLSX () {
        ...
        const readFileLoad = (e, name) => {
          ...
          console.log('result', result.replace(/^.+?(?=\{)/, '').replace(/, *?\}$/, '}'));
          const data = JSON.parse(result.replace(/^.+?(?=\{)/, '').replace(/, *?\}$/, '}'))
          // 把js内容压入表格
          for (let key in data) {
            const tableArray = []
        ...
        reader.onload = (e) => {
          ...
          for (let i = 1; i < myjson.length; i++) {
            ...
            for (let j = 0; j < languageKey.length; j++) {
              ...
              this.getPositionSetData(totalData[languageKey[j].name], keyObj, myjson[i][languageKey[j].index])
            }
          }
